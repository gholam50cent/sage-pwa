...
const STR = {
  ...
  delete_confirm: ["This action cannot be undone. Are you sure?", "این عمل قابل بازگشت نیست. مطمئن هستید؟"],
  delete: ["Delete", "حذف"],
  ...
  restore_failed: ["Restore failed: invalid file format", "بازیابی ناموفق: فرمت فایل نامعتبر"],
  ...
  date_error: ["End date must be after start date", "تاریخ پایان باید بعد از شروع باشد"],
  duration_error: ["Period duration cannot exceed 14 days", "مدت پریود نباید بیشتر از ۱۴ روز باشد"],
  symptom_added: ["Symptom added to today's log", "علامت به لاگ امروز اضافه شد"]
};
...
const Storage = {
  ...
  loadRecords() { return this._safe('sage_records', []); },
  saveRecords(records) { return this._save('sage_records', records); },
  loadDayLogs() { return this._safe('sage_daylogs', {}); },
  saveDayLogs(logs) { return this._save('sage_daylogs', logs); },
  getSettings() { return this._safe('sage_settings', {}); },
  saveSettings(s) { return this._save('sage_settings', s); },
  getHandledSuggestions() { return this._safe('sage_suggestions', {}); },
  saveHandledSuggestions(obj) { return this._save('sage_suggestions', obj); }
};
...
// ========================= مدیریت مودال (رفع باگ بسته نشدن) =========================
...
// ========================= دیالوگ تأیید سفارشی (جایگزین confirm بومی) =========================
...
function confirmOk() {
  const cb = state._confirmCallback;
...
function renderCalendarGrid(analysis) {
  ...
  // اصلاح: بررسی اول و آخر ماه برای نام شمسی صحیح‌تر
  const firstJ = gregorianToJalali(y, m+1, 1);
  const lastJ = gregorianToJalali(y, m+1, daysInMonth);
  const displayJ = (firstJ.month === lastJ.month) ? firstJ : gregorianToJalali(y, m+1, 15);
  const mname = currentLang===1 ? MONTH_NAMES_FA[displayJ.month-1] : MONTH_NAMES_EN[displayJ.month-1];
  const displayYear = displayJ.year;

  // اصلاح: شروع هفته بر اساس زبان
  const firstOfMonth = new Date(y, m, 1);
  let firstDayIdx = firstOfMonth.getDay(); // 0=Sun
  let weekDays;
  if (currentLang === 1) {
    firstDayIdx = (firstDayIdx + 1) % 7; // شنبه=0
  ...
  cells.forEach(function(d) {
    ...
    const isPeriod = state.records.some(function(r) { return millis >= r.start && millis <= (r.end != null ? r.end : r.start); });
...
function renderHistoryTab() {
  ...
  const sorted = [...records].sort(function(a,b) { return b.start - a.start; });
  const itemsHtml = sorted.map(function(r) {
...
function renderStatsTab(analysis) {
  ...
  const completed = state.records.filter(function(r) { return r.end != null; }).length;
  html += `
    <div class="stat-grid">
  ...
  if (predictability.length >= 5) {
    const segs = predictability.map(function(p) { return `<div class="timeline-seg" style="background:rgba(143,166,142,${0.15+p.score*0.85})"></div>`; }).join('');
    html += `
      <div class="card">
  ...
  if (correlations.length > 0) {
    const rows = correlations.map(function(c) {
...
function acceptSuggestion(id) {
  markHandled(id, true);
  // اصلاح: اضافه کردن علامت به لاگ امروز
  const patterns = detectSymptomPatterns(state.records, state.dayLogs);
  const pattern = patterns.find(function(p) { return suggestionId(p) === id; });
  if (pattern) {
    const today = todayMillis();
    const log = state.dayLogs[today] || { flow: 0, pain: 0, mood: -1, symptoms: [], note: '' };
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-card">
        ...
        <div style="display:flex;gap:8px;align-items:center;margin-bottom:16px">
          ...
          ${state._tempEnd ? `<button onclick="state._tempEnd=null;renderModalOnly()" style="color:var(--accent);font-size:12px;white-space:nowrap">${t('clear')}</button>` : ''}
        </div>
...
function saveAddEdit() {
  // اعتبارسنجی: تاریخ پایان باید بعد از شروع باشد
  if (state._tempEnd != null && state._tempEnd < state._tempStart) {
    showToast(t('date_error'), 'error');
    return;
  }
  // اعتبارسنجی: مدت پریود نباید بیشتر از ۱۴ روز باشد
  if (state._tempEnd != null) {
    const duration = Math.round((state._tempEnd - state._tempStart) / DAY_MS) + 1;
    if (duration > 14) {
...
function renderDayDetailModal() {
  ...
  const isPeriodDay = state.records.some(function(r) { return millis >= r.start && millis <= (r.end != null ? r.end : r.start); });
  const flowLabels = [t('flow_none'),t('flow_light'),t('flow_medium'),t('flow_heavy')];

  let flowChips = flowLabels.map(function(lbl, i) {
  ...
  let moodRow = MOOD_KEYS.map(function(k, i) {
  ...
  let symptomGrid = SYMPTOM_KEYS.map(function(k) {
  ...
  return `
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-card">
        ...
        <label class="field-label">${t('pain')} <span style="float:${currentLang===1?'left':'right'};color:var(--accent);font-weight:700">${numStr(state._tempPain)}/${numStr(10)}</span></label>
    ...
    <div class="modal-overlay" onclick="if(event.target===this)closeModal()">
      <div class="modal-card">
        ...
        <div class="settings-option-row">
          ${langs.map(function(l,i){return `<div class="settings-option ${state.langMode===i?'selected':''}" onclick="setLangMode(${i})">${l}</div>`;}).join('')}
        </div>

        <div class="settings-section-label">${t('appearance')}</div>
        <div class="settings-option-row">
          ${themes.map(function(l,i){return `<div class="settings-option ${state.themeMode===i?'selected':''}" onclick="setThemeMode(${i})">${l}</div>`;}).join('')}
        </div>
...
function handleRestoreFile(event) {
  ...
  // استفاده از دیالوگ تأیید سفارشی به جای confirm بومی
